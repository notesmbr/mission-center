...
const LOG_PATH = path.join(WORKSPACE_ROOT, 'trader.log')
const PATH_HINT = 'trader.log'
const MAX_LINES = 200
  ...
  try {
    ...
    if (!fs.existsSync(LOG_PATH)) {
      return res.status(200).json({
        dataSource: 'local_log',
        available: false,
        reason: 'Trader log not found on this host.',
        path: PATH_HINT,
    ...
    return res.status(200).json({
      dataSource: 'local_log',
      available: false,
      reason: `Failed to read trader log: ${err?.message || String(err)}`,
      path: PATH_HINT,
      lastUpdated: new Date().toISOString(),
    })
  }
